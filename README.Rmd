...
```
The mean number of steps taken in a day is approximately `r as.integer(mn)`, while the median of the number of daily steps is `r md`, as also shown in this updated histogram.
```{r}
hist(as.numeric(daily_totals$total_steps), breaks = 20, xlim = c(0,25000), ylim = c(0,12), col = "red",
  main = "Histogram of number of steps in a day \n (Oct/Nov 2012)",
  xlab = "Number of steps in a day")
...
The time interval with the highest total number of steps occurs at `r t$hour`:`r t$min`. `r y` steps were taken in total during that time interval over the observed period.
...
We can now draw a histogram of total daily steps, and the mean and median number of daily steps, using the updated data set, to see the impact of replacing NA values with calculated values.
```{r}
## Calculate the total number of steps by date
